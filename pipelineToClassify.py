...
def specify_models():
	...
	lis = list([nbayes,knear, svc_linear, sv_radial, loglas, sgdc, decis_tree, ranfor, extrerantree])
...
'''
	...
	grid_obj = GridSearchCV(model_dict['class'], param_grid, scoring=metric, cv=k, refit='acc')
	grid_obj = grid_obj.fit(X, y)
	best_parameters = grid_obj.best_params_
	print(best_parameters)
...
if __name__ == '__main__':
	...
	if not precreated_file_ready:
		file_path = "D:\\fact_checking\\"
		file_name = "config_CBD_with_static_filters_11_07_model_6_embeddings.tsv"
		...
		while line:
			...
			if not app[0].startswith("<http://data.gesis.org/claimskg/creative_work/"):
				line = f.readline()
				continue
		...
		df = pd.read_csv("D:\\fact_checking\\dataframe.csv", sep='\t', encoding='utf-8')
	...
	for model_dict in model_list:
		...
		print(make_cls(model_dict, x_feature_vectors, y_class_vector, my_scores, k=10))
