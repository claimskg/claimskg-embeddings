...
if __name__ == '__main__':
    ...
    local_path = None
    sparql_kg = None
    if not args[0].startswith("e") and not args[0].startswith("f"):
    ...
    if local_path is not None:
        print("Loading ClaimsKG from local file..")
        basic_g.load(local_path, format="turtle")
    claim_id_set = set()
    reviewRatingList = ["1", "2", "3"]  # FALSE, MIXTURE, TRUE

    if not local_path:
        print("Loading claims kg subset from SPARQL...")
    ...
    if extend:
        ...
        mentions_list.sort()
        for resource_uri in tqdm(mentions_list):
            cont += 1
        ...
        extended_g.serialize("ClaimsKG_E.rdf", format="application/rdf+xml")
    else:
        extended_g.serialize("ClaimsKG_B.rdf", format="application/rdf+xml")
    ...
    if filter:
        ...
        for uri in tqdm(list_of_predicate_to_remove):
        ...
        extended_g.serialize("graph_CBD_with_static_filters.rdf",
                             format="application/rdf+xml")  # .encode("utf-8")
