...
def get_keywords_triples_from_claimKG(ratingValue_, g, claim_id_set, sparql):
...
def get_mentions_triples_from_claimKG(ratingValue_, g, claim_id_set, sparql):
    # create dict claim uri - keywords
    sparql.setQuery("""    PREFIX schema: <http://schema.org/>
                           PREFIX dbr: <http://dbpedia.org/resource/>
...
def get_author_triples_from_claimKG(ratingValue_, g, claim_id_set, sparql):
    # create dict claim uri - keywords
    sparql.setQuery("""    PREFIX schema: <http://schema.org/>
                           PREFIX dbr: <http://dbpedia.org/resource/>
...
def get_publication_date_triples_from_claimKG(ratingValue_, g, claim_id_set, sparql):
    # create dict claim uri - keywords
    sparql.setQuery("""    PREFIX schema: <http://schema.org/>
                           PREFIX dbr: <http://dbpedia.org/resource/>
...
def get_sameAs_triples_from_claimKG(ratingValue_, g, claim_id_set, sparql):
    # create dict claim uri - keywords
    sparql.setQuery("""    PREFIX schema: <http://schema.org/>
                           PREFIX dbr: <http://dbpedia.org/resource/>
    ...
    for result in results["results"]["bindings"]:
        ...
        if "claim1" in result:
            ...
            g.add((rdflib.term.URIRef(claim_id),
                   rdflib.term.URIRef("http://www.w3.org/TR/2004/REC-owl-semantics-20040210/#owl_sameAs"),
